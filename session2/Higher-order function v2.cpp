...
template <class Fun1, class Fun2> decltype(auto) compose(Fun1 f, Fun2 g) {
  return [&](auto a) { return f(g(a)); };
}

template <class Fun> auto set1st(Fun f, int x) {
  return [&f, x](int y) { return f(x, y); };
}

template <class Fun> auto set2nd(Fun f, int y) {
