...
struct Table<T, 0> {
    ...
    // begin() and end() for iterators (return null pointers for empty Table)
    T* begin() {
        return nullptr;
    }
...
int main() {
    ...
    Table<double, 0> empty_table;
    std::cout << "Size of empty table: " << empty_table.size() << "\n";
    if (empty_table.begin() == empty_table.end()) {
        std::cout << "Empty table has no elements to iterate over.\n";
    }
